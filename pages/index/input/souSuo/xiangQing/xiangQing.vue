<template>
	<view>
		...
		<view class="text">
			<text>{{shuJu.title}}\n</text>
			<text style="color: #FC4369;">￥{{shuJu.skuList[leiXing].price/10}}</text>
			<text style="text-decoration: line-through;color: #C0C0C0;"
				v-if="shuJu.price!=shuJu.originalPrice">￥{{shuJu.skuList[leiXing].originalPrice/10}}</text>
			<text>\n</text>
			<text style="width: 50%;display: inline-block;">销量：{{shuJu.sales}}</text>
			<text style="width: 50%;display: inline-block;">库存：{{shuJu.stock}}</text>

			<view style="width: 100%;" @click="xuang=true">
		...
		<view class="page-bottom">
			...
			<view class="p-b-btn" :class="{active: goods.collect}" style="width: 14%;"
				v-if="!shouChang" @click="shou">
				<text class="iconfont">&#xe62a;</text>
				<text>\n收藏</text>
			</view>
