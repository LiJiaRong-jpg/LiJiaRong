<template>
	<view>
		...
		<view
			style="clear: both;position: fixed;bottom: 0;width: 100%;border-top: #808080 solid 1px;background-color: white;">
		...
		<view v-if="zhiFu==true"
			...
			<view style="text-align: center;">
				<text>支付金额</text>
				<text v-if="totalPrice!=null">\n¥{{totalPrice/100}}</text>
				<text v-if="totalPrice==null">\n¥{{ding.totalPrice/100*ding.skuList[0].num}}</text>
			</view>
	...
	export default {
		...
		onShow() {
			...
			uni.getStorage({
				key: 'orderNo',
				success: function(res) {
					that.orderNo = res.data[0]
					that.totalPrice = res.data[1]
					that.zhiFu = true
					uni.removeStorage({
						key: 'orderNo'
					})
